...
router.post('/coupons/validate', async (req, res) => {
    try {
        ...
        if (activeSeasonalOffer) {
            let discount = 0;
            const markedPrice = activeSeasonalOffer.fixedPrice || 0;

            if (activeSeasonalOffer.discountType === 'AMOUNT') {
                // Flat amount off the marked price
                discount = Math.min(activeSeasonalOffer.discountValue, markedPrice);
            } else {
                // Percent off the marked price (if fixedPrice exists)
                // Otherwise fall back to percent of order total
                if (markedPrice > 0) {
                    discount = (markedPrice * activeSeasonalOffer.discountValue) / 100;
                } else {
                    discount = (orderTotal * activeSeasonalOffer.discountValue) / 100;
                }
            }

            discount = Math.round(discount * 100) / 100; // round to 2dp
            const finalPrice = markedPrice > 0 ? Math.max(0, markedPrice - discount) : null;

            return res.status(200).json({
                success: true,
                discount,
                message: `Seasonal Offer Applied! ${activeSeasonalOffer.discountType === 'AMOUNT' ? `₹${activeSeasonalOffer.discountValue} OFF` : `${activeSeasonalOffer.discountValue}% OFF`} on marked price₹${markedPrice}`,
                markedPrice: markedPrice || null,
