                ...
                <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div className="modal-content" style={{ background: '#fff', padding: '30px', borderRadius: '16px', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto' }}>
                        ...
                        <form onSubmit={handleSave}>
                            ...
                            <div className="form-group" style={{ marginBottom: '15px' }}>
                                <label style={{ fontWeight: '800', color: '#555', fontSize: '0.85rem' }}>Product Media (Thumbnail)</label>
                                <div onClick={() => document.getElementById('file-up').click()} 
                                     style={{ border: '2px dashed #ddd', padding: '24px', textAlign: 'center', borderRadius: '14px', cursor: 'pointer', background: '#fcfcfc', transition: 'border-color 0.3s' }}>
                                    {uploadingImage ? (
                                        <div style={{ padding: '10px' }}>
                                            <i className="fas fa-spinner fa-spin" style={{ fontSize: '1.5rem', color: 'var(--primary)' }}></i>
                                            <p style={{ marginTop: '8px', fontSize: '0.8rem' }}>Uploading...</p>
                                        </div>
                                    ) : (
                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                                            {formData.image && (
                                                <img 
                                                    ...
                                                    style={{ height: '100px', maxWidth: '100%', borderRadius: '10px', objectFit: 'contain', boxShadow: '0 4px 15px rgba(0,0,0,0.1)' }} 
                                                    onError={(e) => { e.target.onerror = null; e.target.src = 'https://images.unsplash.com/photo-1541745537411-b8046f4d5092?w=100'; }}
                                                />
                                            )}
                                            <div style={{ padding: '5px' }}>
                                                <i className="fas fa-camera" style={{ fontSize: '1.5rem', color: '#ccc', marginBottom: '4px', display: 'block' }}></i>
                                                <p style={{ margin: 0, fontWeight: '700', color: '#666', fontSize: '0.85rem' }}>
                                                    {formData.image ? 'Click to RENEW / Change Image' : 'Click to Upload Product Image'}
                                                </p>
                                                <span style={{ fontSize: '0.72rem', color: '#999', marginTop: '4px', display: 'block' }}>Recommended Size: 542 x 592px (width x height)</span>
                                            </div>
                                        </div>
                                    )}
                                    <input type="file" id="file-up" style={{ display: 'none' }} onChange={handleImageUpload} accept="image/*" />
                                </div>
                                <input type="text" value={formData.image} onChange={e => setFormData({...formData, image: e.target.value})} placeholder="Direct Image URL or Path" style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #eee', marginTop: '8px', fontSize: '0.75rem' }} />
                            </div>
