...
const CartDrawer = () => {
    ...
    // Auto-clear coupon/discount when cart changes
    useEffect(() => {
        if (discount > 0) {
            setDiscount(0);
            setCouponCode('');
            setCouponMsg(cartItems.length > 0 ? '⚠️ Cart updated. Coupon removed.' : '');
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.markedPrice && data.finalPrice !== null) {
                    setCouponMsg(`✅ ${data.message}. ₹${data.markedPrice} → ₹${Math.round(data.finalPrice)}`);
                } else {
                    setCouponMsg(`✅ ${data.message} (-₹${Math.round(data.discount)})`); 
                }
            }
                            ...
                            <>
                                ...
                                <div style={{ 
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                        <span style={{ fontSize: '0.85rem', fontWeight: '800', color: deliveryFee === 0 ? '#16a34a' : '#111' }}>
                                            {deliveryFee === 0 ? '✨ FREE Delivery Unlocked!' : `Delivery Charge: ₹${deliveryFee}`}
                                        </span>
                                        {deliveryFee > 0 && cartSubtotal > 0 && (
                                            <span style={{ fontSize: '0.75rem', fontWeight: '700', color: '#B71C1C' }}>
                                    ...
                                    <div style={{ width: '100%', height: '8px', background: '#e0e0e0', borderRadius: '10px', overflow: 'hidden' }}>
                                        <div style={{ 
                                            width: `${Math.min(100, (cartSubtotal / deliverySettings.threshold) * 100)}%`, 
                                            height: '100%', 
                                            background: deliveryFee === 0 ? '#16a34a' : '#B71C1C',
